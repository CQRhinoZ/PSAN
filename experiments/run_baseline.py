...
from models.datasets import AirQuality, MetrLA, PemsBay, PeMS03, PeMS04, PeMS07, PeMS08, SolarBenchmark, Elergone,\
...
def parse_args():
    ...
    parser.add_argument('--mice-n-features', type=int, default=10)
...
class MatrixFactorizationImputer(Imputer):
    ...
    def __init__(self, rank=10, loss='mae', verbose=1):
        method = MatrixFactorization(rank=rank,  verbose=verbose)
...
def get_dataset(dataset_name):
    ...
    if dataset_name == 'pems08':
        dataset = add_missing_values(PeMS08(mask_zeros=True), p_fault=p_fault, p_noise=p_noise,
                                  min_seq=12, max_seq=12 * 4, seed=56789)
    elif dataset_name == 'pems04':
        dataset = add_missing_values(PeMS04(mask_zeros=True), p_fault=p_fault, p_noise=p_noise,
                                  min_seq=12, max_seq=12 * 4, seed=56789)
    else:
    ...
    train_slice = np.zeros(len(dataset.numpy())).astype(bool)
    train_slice[:-int(0.2 * len(dataset))] = True
...
def run(imputer, dataset, train_slice):
    ...
    if args.in_sample:
        ...
        mask_train = mask_train.reshape(mask_train.shape[0], mask_train.shape[1])
        y_hat = imputer.predict(x_train, mask_train)
        y_hat = y_hat.reshape(y_hat.shape[0], y_hat.shape[1], 1)
        y_hat = y_hat[test_slice]
    else:
...
if __name__ == '__main__':
    ...
    for ds_name in args.datasets:
        ...
        # args.adj = dataset.get_connectivity(
        #     method='binary',
        #     threshold=0.1,
        #     include_self=False,
        #     layout='dense',
