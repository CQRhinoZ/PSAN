...
class RNNImputer(nn.Module):
    ...
    def _preprocess_input(self, x, x_hat, m, u):
        if self.detach_inputs:
            x_p = torch.where(m.type(torch.bool), x, x_hat.detach())
        else:
            x_p = torch.where(m.type(torch.bool), x, x_hat)

        # print(x_p.shape, u.shape)
        if u is not None:
    ...
    def forward(self, x, mask, u=None, return_hidden=False):
        ...
        x = torch.where(mask.type(torch.bool), x, torch.zeros_like(x))
...
class BiRNNImputer(nn.Module):
    ...
    def forward(self, x, mask, u=None, return_hidden=False):
        ...
        # print(x_hat.shape, x_hat_fwd.shape, x_hat_bwd.shape)
        return x_hat.unsqueeze(dim=-1), x_hat_fwd.unsqueeze(dim=-1), x_hat_bwd.unsqueeze(dim=-1)
