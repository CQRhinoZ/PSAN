...
from tsl.nn.metrics import MaskedMetric, MaskedMAE, MaskedMSE, MaskedMRE, MaskedMAPE
from tsl.nn.utils import casting
from tsl.utils import parser_utils, numpy_metrics
from tsl.utils.parser_utils import ArgParser

from models.imputation_ops import add_missing_values
from models.baselines import SAITS, TransformerModel, BRITS, SPINModel, RGAINNet, ImputeFormerModel
from tsl.nn.models.imputation import GRINModel
from models.imputers import SPINImputer, SAITSImputer, BRITSImputer, RGAINFiller, ImputeFormerImputer, PSANImputer
from models.models import PSANModel
from models.scheduler import CosineSchedulerWithRestarts
from models.datasets import  MetrLA, PemsBay, PeMS04, PeMS08

def get_model_classes(model_str):
    if model_str == 'psan':
        model, filler = PSANModel, PSANImputer
    else:
...
def run_experiment(args):
    ...
    if args.model_name == 'rgain':
        imputer.load_state_dict(torch.load(checkpoint_callback.best_model_path,
    ...
    if args.model_name == 'rgain':
        y_true, y_hat, mask = imputer.predict_loader(dm.test_dataloader(), return_mask=True)
    else:
    ...
    check_mape = numpy_metrics.masked_mape(y_hat, y_true, mask) * 100
    check_mse = numpy_metrics.masked_mse(y_hat, y_true, mask)
    check_mre = numpy_metrics.masked_mre(y_hat, y_true, mask) * 100

    print(f'Test MAE: {check_mae:.2f}')
    print(f'Test MAPE: {check_mape:.2f}')
    print(f'Test MSE: {check_mse:.2f}')
    print(f'Test MRE: {check_mre:.2f}')
    return y_hat
