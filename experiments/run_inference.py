...
from models.baselines import SAITS, TransformerModel, BRITS, SPINModel, ImputeFormerModel
from models.imputers import SPINImputer, SAITSImputer, BRITSImputer, ImputeFormerImputer
from models.models import PSANModel
from models.imputers import PSANImputer
...
def run_experiment(args):
    ...
    for seed in seeds:
        ...
        rmse.append(check_rmse)
        check_mape = numpy_metrics.masked_mape(y_hat, y_true, mask)
        mape.append(check_mape)
        check_mre = numpy_metrics.masked_mre(y_hat, y_true, mask)
        mre.append(check_mre)
        print(f'SEED {seed} - Test MAE: {check_mae:.2f}')
        print(f'SEED {seed} - Test RMSE: {check_rmse:.2f}')
        print(f'SEED {seed} - Test MAPE: {check_mape:.2f}')
        print(f'SEED {seed} - Test MRE: {check_mre:.2f}')

    print(f'MAE over {len(seeds)} runs: {np.mean(mae):.2f}±{np.std(mae):.2f}')
    print(f'RMSE over {len(seeds)} runs: {np.mean(rmse):.2f}±{np.std(rmse):.2f}')
    print(f'MAPE over {len(seeds)} runs: {np.mean(mape):.2f}±{np.std(mape):.2f}')
    print(f'MRE over {len(seeds)} runs: {np.mean(mre):.2f}±{np.std(mre):.2f}')
